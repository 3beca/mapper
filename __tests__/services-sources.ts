...
import {
    buildSourcesService, SourcesService, Source
...
describe(
    'getSources should',
    () => {
        let dbClient: MongoClient, collection: Collection, service: SourcesService;
        beforeAll(
            async () => {
        ...
        it(
            ...
            async () => {
                expect.assertions(1);
                const service = buildSourcesService({} as Collection);
    ...
    () => {
        ...
        it(
            ...
            async () => {
                ...
                try {
                    await service.insertSource({} as Source);
                }
                catch (error) {
                    expect(typeOfError(error, ERROR_DATABASE.type)).toBe(true);
                }
        ...
        it(
            ...
            async () => {
                expect.assertions(1);
                const source = null as unknown as Source;
                try {
                    await service.insertSource(source);
                }
                catch (error) {
                    expect(typeOfError(error, ERROR_SOURCE_FORMAT.type)).toBe(true);
                }
            }
        );
        ...
        it(
            ...
            async () => {
                expect.assertions(1);
                const source = undefined as unknown as Source;
                try {
                    await service.insertSource(source);
                }
                catch (error) {
