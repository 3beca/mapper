...
import { buildTargetsService, TargetsService } from '../src/services/targets';
import {Collection} from 'mongodb';
import { SourcePersisted } from '../src/services/sources';
import { buildHttpEngineService, HttpEngineService } from '../src/services/http-engine';
    ...
    () => {
        ...
        it(
            ...
            async () => {
                ...
                const fakeCollection = { findOne: jest.fn().mockRejectedValue(new Error('Mongo error')) } as unknown as Collection;
                const mappingsService = buildMappingsService(fakeCollection);
                const targetsService: TargetsService = buildTargetsService(fakeCollection);
                const httpEngineService: HttpEngineService = buildHttpEngineService();

                try {
                    await buildMapperService(mappingsService, targetsService, httpEngineService)(source, context);
                }
                catch (error) {
